...
This reference is based on Apinni v1.0.0 (as of September 25, 2025). For updates, check the [GitHub repository](https://github.com/apinni/apinni).
</Admonition>
...
import { loadApinniConfig } from 'apinni';
...
import { extractDecoratorArgValue } from 'apinni';
...
```ts
import { GenerationContext } from 'apinni';
...
```ts
import { SwaggerPlugin } from 'apinni';
...
```ts
import { ProxySwaggerPlugin } from 'apinni';
...
For source code and contributions, visit the [Apinni GitHub](https://github.com/apinni/apinni). Contact the [community](https://community.apinni.dev) for support.
</Admonition>